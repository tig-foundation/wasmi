use super::Executor;
use crate::{core::UntypedVal, engine::bytecode::UnaryInstr, store::StoreInner, Error};
...
macro_rules! impl_fallible_conversion_impls {
    ( $( (Instruction::$var_name:ident, $fn_name:ident, $op:expr) ),* $(,)? ) => {
        $(
            #[doc = concat!("Executes an [`Instruction::", stringify!($var_name), "`].")]
            #[inline(always)]
            pub fn $fn_name(&mut self, store: Option<&mut StoreInner>, instr: UnaryInstr) -> Result<(), Error> {
                self.try_execute_unary(store, instr, $op)
            }
        )*
    };
}
